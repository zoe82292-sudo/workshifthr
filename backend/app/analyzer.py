...
def _read_csv(content: bytes, filename: str = "upload.csv") -> pd.DataFrame:
    if _looks_like_export_file(content, filename):
        raise ValueError(
            "This file looks like a WorkShift HR results export, not an employee compensation "
    ...
    raise ValueError(
        ...
        "(not a WorkShift HR export). In Excel: File → Save As → CSV UTF-8, or upload .xlsx."
    )
